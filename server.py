...
def handle_input(data):
    ...
    if session_id in processes:
        ...
        try:
            ...
            socketio.emit("output", {"response": f"Error sending input: {str(e)}"}, room=session_id)
    else:
        socketio.emit("output", {"response": "No running command to send input to."}, room=session_id)
