...
ALLOWED_COMMANDS = {"ls", "cat", "echo", "python", "pip", "python3", "pwd"}  # Add only safe commands
...
def handle_command(data):
    ...
    full_command = data.get("command", "").strip()  # Get full command
    base_command = full_command.split(" ")[0]  # Extract base command

    if base_command not in ALLOWED_COMMANDS:
        emit("output", {"response": f"ERROR: Command '{base_command}' is not allowed."})
        return
    ...
    try:
        ...
        processes[data.get("session_id", "global")] = process  # Store process

        for line in iter(process.stdout.readline, ""):
            emit("output", {"response": line.strip()})
            socketio.sleep(0.1)

        for line in iter(process.stderr.readline, ""):
            emit("output", {"response": f"ERROR: {line.strip()}"})
            socketio.sleep(0.1)

        process.stdout.close()
        process.stderr.close()
        process.wait()
        del processes[data.get("session_id", "global")]  # Remove process after completion

    except Exception as e:
        emit("output", {"response": f"Error executing command: {str(e)}"})
...
def stop_command(data):
    ...
    if session_id in processes:
        ...
        process.wait()  # Wait for process to stop
        del processes[session_id]  # Remove from tracking
        emit("output", {"response": "Command stopped successfully."})
    else:
        emit("output", {"response": "No running command to stop."})
...
def edit_file():
    ...
    if not file_path or not filename.endswith('.py'):
