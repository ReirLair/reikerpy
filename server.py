...
def stop_command(data):
    """Stop a running command and all its subprocesses safely."""
    ...
    if session_id in processes:
        ...
        try:
            ...
            os.killpg(pgid, signal.SIGTERM)  # Graceful stop

            # Ensure all child processes are stopped
            parent = psutil.Process(process.pid)
            children = parent.children(recursive=True)
            for child in children:
                child.terminate()  # Send SIGTERM to child processes
            ...
            process.wait(timeout=3)  # Ensure main process stops
            del processes[session_id]

            socketio.emit("output", {"response": "Command stopped successfully."}, room=session_id)
        except psutil.NoSuchProcess:
            socketio.emit("output", {"response": "Process already stopped."}, room=session_id)
        except Exception as e:
