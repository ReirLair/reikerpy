    ...
    def run_command(sid):
        try:
            ...
            while True:
                ...
                if output:
                    socketio.emit("output", {"response": output}, room=sid)
                if error:
                    socketio.emit("output", {"response": f"ERROR: {error}"}, room=sid)
            ...
            # Remove the process from tracking once it finishes
            processes.pop(sid, None)

    # Run the command in a separate thread and track it
