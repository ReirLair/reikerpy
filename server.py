...
def handle_command(data):
    ...
    def run_command():
        try:
            process = subprocess.Popen(
                ...
                start_new_session=True  # Runs independently from Flask
            )
            ...
            while True:
                ...
                if output:
                    emit("output", {"response": output.strip()})
                if error:
                    emit("output", {"response": f"ERROR: {error.strip()}"})
    ...
    # Run the command in a separate thread to prevent blocking
