...
def handle_command(data):
    ...
    # Prevent duplicate commands from running in the same session
    ...
    def run_command(sid):
        try:
            process = subprocess.Popen(
                ...
                stderr=subprocess.PIPE,  # Separate stderr again
                universal_newlines=True,
                bufsize=1,  # Force real-time output flushing
                start_new_session=True
            )
            processes[sid] = process  # Store process
            ...
            while True:
                ...
                if output:
                    socketio.emit("output", {"response": output.strip()}, room=sid)
                if error:
                    socketio.emit("output", {"response": f"ERROR: {error.strip()}"}, room=sid)

                if process.poll() is not None:  # Process completed
                    break  

                socketio.sleep(0.1)  # Allow other events to be processed
            ...
            # Ensure the process is removed from tracking after completion
            processes.pop(sid, None)

    # Run the command in a separate thread
    threading.Thread(target=run_command, args=(request.sid,), daemon=True).start()


@socketio.on('stop')
def stop_command(data):
    """Stop a running command."""
    session_id = data.get("session_id", request.sid)  # Use request.sid if session_id is missing
