...
def stop_command(data):
    ...
    session_id = data.get("session_id", request.sid)  # Use request.sid if missing
    print(f"Stopping process for session_id: {session_id}")

    if session_id in processes:
        process = processes[session_id]
        try:
            # Use psutil to get child processes
            parent = psutil.Process(process.pid)
            children = parent.children(recursive=True)  # Get all child processes

            # Terminate all child processes first
            for child in children:
                child.terminate()  # Send SIGTERM to each child

            # Ensure all child processes are fully stopped
            _, still_alive = psutil.wait_procs(children, timeout=3)
            for child in still_alive:
                child.kill()  # Force kill if they didn't terminate

            # Now stop the main process
            if process.poll() is None:  # If still running
                process.terminate()  # Send SIGTERM
                process.wait(timeout=3)  # Wait for it to exit
            ...
            socketio.emit("output", {"response": f"Error stopping command: {str(e)}"}, room=session_id)
    else:
