...
def stop_command(data):
    """Stop a running command and all its subprocesses."""
    session_id = data.get("session_id", request.sid)

    if session_id in processes:
        process = processes[session_id]
        try:
            # Get the process group ID (pgid) and kill entire group
            pgid = os.getpgid(process.pid)
            os.killpg(pgid, signal.SIGTERM)

            # Use psutil to find and terminate all child processes
            parent = psutil.Process(process.pid)
            children = parent.children(recursive=True)
            for child in children:
                child.terminate()  # Send SIGTERM to each child process
